...
class UnicodeWordClassifier:
    ...
    regex_latin_extended = regex.compile(
        "^[\p{InLatin-1_Supplement}\p{InBasic_Latin}\p{InLatin_Extended-A}\p{InLatin_Extended-B}]+$")
    regex_arabic = regex.compile("^[\p{Arabic}]+$")
    regex_greek = regex.compile("^[\p{Greek}]+$")
    regex_cyrillic = regex.compile("^[\p{Cyrillic}]+$")
    regexes = [('BASIC_LATIN', regex_basic_latin), ('LATIN_SUPPLEMENT', regex_latin_supplement),
               ('LATIN_EXTENDED', regex_latin_extended),
...
class LMFluencyFilter:
    ...
    def score(self, sentence: str):
        ...
        # Normalize score
        return sum(raw_scores) / (sum([len(s.split()) for s in processed_sents]) + len(
            processed_sents))  # We divide by total number of tokens + 1 for each sentence (taken from kenlm perplexity method)
...
class DualLMFluencyFilter:
    ...
    def score(self, sentence_sl: str, sentence_tl: str):
        return self.scoring_stats.perplexity_to_score(
            self.sl_filter.score(sentence_sl) + self.tl_filter.score(sentence_tl))

    def train(self, lm_train_sl: str, lm_train_tl: str, clean_sl: str, clean_tl: str, noisy_sl: str, noisy_tl: str,
              lm_out_sl: str, lm_out_tl: str) -> DualLMStats:
        try:
            self.sl_filter.train_lm(lm_train_sl)
            self.tl_filter.train_lm(lm_train_tl)
            clean_mean, clean_stddev = LMFluencyFilter.estimate_threshold(self.sl_filter, self.tl_filter, clean_sl,
                                                                          clean_tl)
            noisy_mean, noisy_stddev = LMFluencyFilter.estimate_threshold(self.sl_filter, self.tl_filter, noisy_sl,
                                                                          noisy_tl)
