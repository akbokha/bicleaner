...
regex_other = regex.compile(
    "[^\p{Arabic}\p{Greek}\p{Han}\p{Hangul}\p{Hebrew}\p{Latin}\p{InLatin_Extended-A}\p{InLatin_Extended-B}\p{InLatin-1_Supplement}\p{InBasic_Latin}\p{Hiragana}\p{Katakana}\p{Cyrillic}\p{Devanagari}\p{Malayalam}]")
...
def feature_dict_coverage(slwords, tlwords, dict_stot):
    ...
    return [len(slfound) * 1.0 / (1 + len(slwords)),
            len(slwithtrans) * 1.0 / (1 + len(slfound)) if len(slfound) > 0 else 1.0]
...
def feature_extract(srcsen, trgsen, tokenize_l, tokenize_r, args, dcce_scores=None):
    ...
    features.append(
        feature_dict_qmax(left_sentence_tok, right_sentence_tok, dict12, normalize_by_length, treat_oovs, dict21, fv,
                          qmax_limit))
    features.extend(feature_dict_coverage(left_sentence_tok, right_sentence_tok, dict12))
    features.append(
        feature_dict_qmax(right_sentence_tok, left_sentence_tok, dict21, normalize_by_length, treat_oovs, dict12, fv,
                          qmax_limit))
